...
use crate::pixels::{modifiers::{temporal_progression::TemporalProgression, ModifierChainable, ModifierResult}, pixel::{self, HSV}};
...
{
    pub fn new(start:Instant, duration:Duration, curtain_edge_width:f64, pixel_sets:Vec<(usize,usize)>)->CurtainModifier
    {
...
{
    fn run(&mut self, pixel_strip:&mut crate::pixels::pixelstrip::PixelStrip, params:&crate::pixels::modifiers::ModifierParameters)->crate::pixels::modifiers::ModifierResult {

        let progress=self.progress.get_progress_at_time(&params.time);
        ...
        if progress<=1.0{
            for pixel_set in &self.pixel_sets
            {
                ...
                let curtain_edge_back = progress*pixel_count_f64-inverse_progress*self.curtain_edge_width;
                
                for a in pixel_set.0..=pixel_set.1 {
                    
                    let location_fraction = f64::from(a as u32) / pixel_count_f64;

                    //If the pixels are inverted then invert location fraction
                    let location_fraction = if is_inverted {
                        1.0-location_fraction
                    }
                    else{
                        location_fraction
                    };

                    if location_fraction>curtain_edge_front
                    {
                        //do nothing! curtain hasn't reached pixel yet
                    }
                    else if location_fraction<curtain_edge_back
                    {
                        ...
                        {
                            Some(current)=>{
                                let brightness_coefficient = ((location_fraction-curtain_edge_back)/self.curtain_edge_width*PI).cos();
